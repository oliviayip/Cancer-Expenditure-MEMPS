...
# calymph (lymphoma)
hc201 <- hc201 %>%
  mutate(lymphoma = ifelse(hc201$calymph==1,1,
                       ifelse(hc201$calymph==2,0,NA))) 
...
hc201c <- subset(hc201, select=c(age_cat, age17x, sex, ccnrdi31, ctrtmt31, clstrt31, cbck31, cbckyr31, cftrt31, racev1x, hispanx,
                                 ...
                                 obtotv17, obdrv17, optexp17, optotv17, ipdis17, ipngtd17,iptexp17,cancer))
...
#Coding for the charlson comorbidity index variables:
...
label(test$vcom1) <- 'mi'
label(test$vcom2) <- 'chf'
label(test$vcom3) <- 'pvd'
label(test$vcom4) <- 'cvd'
label(test$vcom5) <- 'dementia'
label(test$vcom6) <- 'cpd'
label(test$vcom7) <- 'rheum'
label(test$vcom8) <- 'pud'
label(test$vcom9) <- 'mld'
label(test$vcom10) <- 'dmwoc'
label(test$vcom11) <- 'hemi'
label(test$vcom12) <- 'renal'
# label(test$vcom13) <- 'malignancy'
label(test$vcom14) <- 'sld'
#label(test$vcom15) <- 'mst'
label(test$vcom16) <- 'aids'
...
hc216_final <- merge(hc201, hc199, by="dupersid",all.x=TRUE)
...
hc216_final <- hc216_final %>% 
  mutate(agescore = ifelse(age17x>17 & age17x<50,0,
                          ifelse(age17x>49 & age17x<60,1,
                                 ifelse(age17x>59 & age17x<70,2,
                                        ifelse(age17x>69 & age17x<80,3,
                                            ifelse(age17x>79 & age17x<100,4,NA))))))

table(hc216_final$agescore) 

#Myocardial infarction +1 point 
table(hc216_final$vcom1)

hc216_final <- hc216_final %>% 
  mutate(MIscore = ifelse(vcom1==1,1,
                      ifelse(vcom1==0,0,NA)))
                                  
                                                

table(hc216_final$MIscore)

#Congestive heart failure +1 point 
table(hc216_final$vcom2)

hc216_final <- hc216_final %>% 
  mutate(CHFscore = ifelse(vcom2==1,1,
                          ifelse(vcom2==0,0,NA)))

table(hc216_final$CHFscore)

#Peripheral vascular disease +1 point 
table(hc216_final$vcom3)

hc216_final <- hc216_final %>% 
  mutate(PVDscore = ifelse(vcom3==1,1,
                           ifelse(vcom3==0,0,NA)))



table(hc216_final$PVDscore)
...
hc216_final <- hc216_final %>% 
  mutate(CVAscore = ifelse(vcom4==1,1,
                           ifelse(vcom4==0,0,NA)))

table(hc216_final$CVAscore)


#Chronic cognitive impairment +1 point 
table(hc216_final$vcom5)

hc216_final <- hc216_final %>% 
  mutate(CIscore = ifelse(vcom5==1,1,
                           ifelse(vcom5==0,0,NA)))

table(hc216_final$CIscore)

#COPD +1 point 
table(hc216_final$vcom6)

hc216_final <- hc216_final %>% 
  mutate(COPDscore = ifelse(vcom6==1,1,
                          ifelse(vcom6==0,0,NA)))

table(hc216_final$COPDscore)

#Connective tissue disease +1 point 
table(hc216_final$vcom7)

hc216_final <- hc216_final %>% 
  mutate(RHEUMscore = ifelse(vcom7==1,1,
                            ifelse(vcom7==0,0,NA)))

table(hc216_final$RHEUMscore)

#Peptic ulcer disease +1 point 
table(hc216_final$vcom8)

hc216_final <- hc216_final %>% 
  mutate(PUDscore = ifelse(vcom8==1,1,
                             ifelse(vcom8==0,0,NA)))

table(hc216_final$PUDscore)

#Liver disease +1 point for mild, +3 points for moderate/severe 

hc216_final <- hc216_final %>% 
  mutate(LIVERscore = ifelse(icd10cdx=="K74" && icd10cdx=="K76",3,
                             ifelse(icd10cdx=="B19" ||icd10cdx=="K74",1,0)))

table(hc216_final$LIVERscore)
...
hc216_final <- hc216_final %>% 
  mutate(HEMIscore = ifelse(vcom11==1,2,
                           ifelse(vcom11==0,0,NA)))

table(hc216_final$HEMIscore)
...
hc216_final <- hc216_final %>% 
  mutate(AIDSscore = ifelse(vcom16==1,6,
                            ifelse(vcom16==0,0,NA)))

table(hc216_final$AIDSscore)


cci_score = agescore + MIscore +
...
ecls_nomiss <- na.omit(hc201c)  #You cannot perform propensity score matching (PSM) if you have missing data ("NA").                                #This generates a new dataframe "ecls_nomiss" that is a copy of your existing dataset removing missing variables 
...
match.it <- matchit(ecls_nomiss$cancer ~ age + sex + marritalstatus + neweducode + povcat      #Replace "mdistress" with the cancer flag variable 
                    + inscov + racev1x + cci, data=ecls_nomiss,
