...
svyby(~cancer, ~racev1x, design=study_dsn, svytotal) #race
...
svytable(~cacervix, design=study_dsn) #insurance coverage
svytable(~cacolon, design=study_dsn) #insurance coverage
svytable(~calung, design=study_dsn) #insurance coverage
svytable(~calymph, design=study_dsn) #insurance coverage
svytable(~camelano, design=study_dsn) #insurance coverage
svytable(~caprosta, design=study_dsn) #insurance coverage
...
study_dsn %>%
  tbl_svysummary(by = cancer, percent = "column", include = c(age_cat,racev1x,neweducode,region,povcat,marritalstatus,inscov))
...
hist(hc216_final$totexp)

#Non-Cancer: 5,738,575
1584644+1882975+296883+1974073      

#Cancer: 7,641,260
1383563+1786831+2812831+1658035

library(sjstats)
#need to install package: sjstats
#Emergency Department, had an inpatient stay, etc. 
m1 <- svyglm.nb(ipdis~factor(cancer)+factor(age_cat)+factor(sex)+factor(hispanx)+factor(marritalstatus)+
                  factor(neweducode)+factor(region)+factor(povcat)+factor(inscov)+factor(racev1x),
                design=study_dsn)
